      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 bg-sky-50">
        <div className="flex justify-between items-center h-16">
          ...
          <nav className="hidden md:flex items-center space-x-8">
            {menuItems.map(item => <button key={item.path} onClick={() => navigate(item.path)} className="flex items-center space-x-2 text-gray-600 hover:text-blush-rose transition-colors duration-200">
                <item.icon size={18} />
          ...
          {/* Mobile menu button */}
          <button onClick={() => setIsMenuOpen(!isMenuOpen)} className="md:hidden p-2 rounded-lg text-gray-600 hover:text-blush-rose hover:bg-gray-100">
            {isMenuOpen ? <X size={24} /> : <Menu size={24} />}
          </button>
        </div>

        {/* Mobile Navigation */}
        {isMenuOpen && <div className="md:hidden py-4 border-t border-gray-200 animate-slide-up">
