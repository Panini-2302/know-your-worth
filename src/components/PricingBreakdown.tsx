...
const PricingBreakdown = ({
  ...
  if (!valuation || !valuation.totalPrice) {
    return <Card className="hover-lift">
  ...
  const pieData = [{
    name: 'Labor Cost',
    value: valuation.laborCost || 0,
    color: '#FFE66D'
  }, {
    name: 'Materials',
    value: valuation.materialCost || 0,
    color: '#D8F3DC'
  }, {
    name: 'Skill Bonus',
    value: Math.max(0, valuation.skillBonus || 0),
    color: '#DDEBF9'
  }, {
    name: 'Complexity',
    value: Math.max(0, valuation.complexityBonus || 0),
    color: '#3A3A3A'
  }, {
    name: 'Market Factors',
    value: Math.max(0, valuation.marketBonus || 0),
    color: '#F9D5D3'
  }, {
    name: 'Revisions',
    value: valuation.revisionCost || 0,
    color: '#D8F3DC'
  }].filter(item => item.value > 0);
  const barData = [{
    category: 'Base Labor',
    amount: valuation.laborCost || 0
  }, {
    category: 'Materials',
    amount: valuation.materialCost || 0
  }, {
    category: 'Skill Premium',
    amount: Math.max(0, valuation.skillBonus || 0)
  }, {
    category: 'Complexity',
    amount: Math.max(0, valuation.complexityBonus || 0)
  }, {
    category: 'Market Demand',
    amount: Math.max(0, valuation.marketBonus || 0)
  }, {
    category: 'Revisions',
    amount: valuation.revisionCost || 0
...
Generated by ArtValue Pro
  ...
  return <Card className="overflow-hidden hover-lift">
      <CardHeader className="bg-gradient-to-r from-blush-rose/30 to-periwinkle-blue bg-sky-50">
        <CardTitle className="flex items-center justify-between font-playfair">
          <div className="flex items-center gap-2">
            <DollarSign className="text-blush-rose" size={24} />
            Pricing Breakdown
          </div>
          <Button onClick={downloadPDF} variant="outline" size="sm" className="border-blush-rose text-blush-rose hover:bg-blush-rose hover:text-white bg-sky-50">
        ...
        <div className="space-y-6">
              ...
              <div className="h-64">
                <ResponsiveContainer width="100%" height="100%">
                  <BarChart data={barData}>
                    ...
                    <XAxis dataKey="category" angle={-45} textAnchor="end" height={60} fontSize={10} />
                    <YAxis fontSize={12} />
                    <Tooltip formatter={value => [`$${value}`, 'Amount']} />
                    <Bar dataKey="amount" fill="#FFE66D" radius={[4, 4, 0, 0]} />
                  </BarChart>
                </ResponsiveContainer>
              </div>
            </div>
          ...
          <div className="space-y-3">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
              ...
              label: 'Skill Premium',
              value: valuation.skillBonus,
              color: 'bg-periwinkle-blue'
            }, {
              label: 'Complexity Bonus',
              value: valuation.complexityBonus,
              color: 'bg-graphite-soft'
            }, {
              label: 'Market Demand',
              value: valuation.marketBonus,
              color: 'bg-blush-rose/70'
            }, {
              label: 'Revision Costs',
              value: valuation.revisionCost,
              color: 'bg-mint-gray/70'
            }].map((item, index) => <div key={index} className="flex items-center justify-between p-3 bg-blush-rose/10 rounded-lg">
